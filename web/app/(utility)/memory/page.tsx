...
const TABS: { key: MemoryFile; labelKey: string; icon: typeof Brain; hintKey: string; placeholderKey: string }[] = [
  {
    key: "summary",
    labelKey: "Memory Summary",
    icon: BookOpen,
    hintKey: "Running summary of the learning journey. Auto-updated after conversations.",
    placeholderKey: "## Current Focus\n- ...\n\n## Accomplishments\n- ...\n\n## Open Questions\n- ...",
  },
  {
    key: "profile",
    labelKey: "Memory Profile",
    icon: User,
    hintKey: "User identity, preferences, and knowledge levels. Auto-updated after conversations.",
    placeholderKey: "## Identity\n- ...\n\n## Learning Style\n- ...\n\n## Knowledge Level\n- ...\n\n## Preferences\n- ...",
  },
];
...
export default function MemoryPage() {
  ...
  const saveMemory = useCallback(async () => {
    ...
    try {
      ...
      setToast(t("{{label}} saved", { label: t(tab.labelKey) }));
    } finally {
      setSaving(false);
    }
  }, [activeTab, editorValue, t, tab.labelKey]);
  ...
  const clearMemory = useCallback(async () => {
    if (!window.confirm(t("Clear {{label}}?", { label: t(tab.labelKey) }))) return;
    setClearing(true);
    try {
      ...
      setData(d);
      setEditors((prev) => ({ ...prev, [activeTab]: d[activeTab] || "" }));
      setToast(t("{{label}} cleared", { label: t(tab.labelKey) }));
    } finally {
      setClearing(false);
    }
  }, [activeTab, t, tab.labelKey]);
    ...
    <div className="h-full overflow-y-auto [scrollbar-gutter:stable]">
      <div className="mx-auto max-w-[960px] px-6 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <p className="max-w-lg text-[12px] text-[var(--muted-foreground)]">{t(tab.hintKey)}</p>
        ...
        {loading ? (
          ...
          <div className="flex min-h-[320px] flex-col items-center justify-center rounded-xl border border-dashed border-[var(--border)] text-center">
            ...
            <p className="text-[14px] font-medium text-[var(--foreground)]">
              {t("No {{label}} yet", { label: t(tab.labelKey).toLowerCase() })}
            </p>
